...
function AnimatePig( ply, velocity )
	...
	if ( key == IN_USE ) then // is taunting/wave
...
hook.Add( "Move", "UCHGhostMove", function( ply, move )if !UCHAnim.IsGhost( ply ) || IsValid( ply.BallRaceBall ) then return end
	...
	if !ply:IsOnGround() then
		...
		if ply:KeyDown( IN_JUMP ) && ply.UCHType == (TYPE_GHOST) then
		...
		if ply:KeyDown( IN_SPEED ) && !ply:KeyDown( IN_JUMP ) && ply.UCHType == (TYPE_GHOST) then
