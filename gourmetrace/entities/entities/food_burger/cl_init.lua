...
function ENT:Draw()
  ...
  local CurAng = self:GetAngles()
	...
	self.csModel:SetPos(self:GetPos() - Vector(0,0, FoodHeight + 20 ))
	self.csModel:SetAngles(Angle(CurAng.p,CurAng.y,CurAng.r + FoodAngle))

	if !self.NoGlow then
		render.SetMaterial( Material("effects/brightglow_y") )
		render.DrawSprite( self:GetPos() - Vector(0,0, FoodHeight ) + Vector(0,0,35), 75 + math.sin(CurTime()*4)*10, 75 + math.sin(CurTime()*4)*10, Color(175,175,175,225) )
	end
