...
function ENT:DrawTranslucent()
	...
	if IsValid( self.PlayerModel ) and (ply ~= LocalPlayer() || !FIRSTPERSON) then
		...
		render.SetBlend(self.Opacity && (1 - self.Opacity) / 255 or 1)
	...
	if IsValid( self.Ball ) then
		...
		render.SetBlend(FIRSTPERSON and ply == LocalPlayer() and 0.4 or self.Opacity and (1 - self.Opacity) / 255 or 1)
