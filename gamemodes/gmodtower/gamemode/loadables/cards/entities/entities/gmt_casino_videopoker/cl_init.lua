        ...
        if pos and ang and scale then
            ...
            local function mob(button)
                if button.circle then
                    return math.Distance(button.x, button.y, pos.x, pos.y) <= button.radius
                else --mouseon(button.x-button.radius,button.y-button.radius,button.radius*2,button.radius*2)
...
function ENT:DrawTranslucent()
    ...
    if cx and self:MouseOn(w / -2, h / -2, w, h) then
        CursorVisible = true
    end
    ...
    ErrorTrace.Call(function()
        ...
        if DrawScreen then
            ...
            elseif self:GetState() == self.States.BEGIN then
                ...
                if (not hand or hand:ToInt() ~= self:GetHandInternal()) and self:GetHandInternal() > 0 then
