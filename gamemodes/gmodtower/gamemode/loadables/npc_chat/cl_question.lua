...
function PANEL:SlidingThink()
	...
	// Center mouse on it (if it's the first one)
	if CENTERMOUSE && self.ID == 1 then
		gui.SetMousePos( self.x + ( self:GetWide() / 2 ), self.y + ( self:GetTall() / 2 ) )
	end
...
function PANEL:QuestionThink()
	...
	for _, v in pairs( Entities ) do
		if v:GetPos():Distance( LocalPos ) <= GTowerNPCChat.NPCMaxTalkDistance then
			return
		end	
	end	
