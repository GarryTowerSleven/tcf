...
function RULE:Think(volume, dsp)
	...
	if IsValid(self.LoopingSound) then
		...
		mp = mp[1] and mp[1]._Media and 1 or 0
...
function RULE:ApplySettings(snd)
	...
	local time = soundscape.GetValueFromVagueObject(self.Table.time) or 5
	...
	if false then
		local soundFile, _ = soundscape.GetSoundInfoFromVagueObject(self.Table.sounds)
