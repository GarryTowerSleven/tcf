...
function PANEL:OpenMenu()
    ...
    if not string.find(CommandId, "-2") then
        --Msg( CommandId )
        table.insert(Menu, {
            ["Name"] = "Send To Vault",
            ["canclose"] = true,
...
function PANEL:DraggingEntThink()
	...
	if ( !self:AllowDrop( NewPos ) or !self:CheckTraceHull() ) then
        GhostEntity:SetColor(Color(255, 100, 100, 190))
    end
end
...
function PANEL:AllowDrop( pos )
	local ply = LocalPlayer()
	local item = self:GetItem() or nil

	if ( ply:GetSetting( "GTAllowInvAllEnts" ) ) then
		return true
	end
	...
	if ( item ) then
		...
		if ( pos && item.AllowDropLocation && ( Location.Get( Location.Find( pos ) ).Name or nil ) == item.AllowDropLocation ) then
			return true
		end
	end

	local room = ply:GetNWInt( "RoomID", nil )

	if ( pos && Location.IsCondo( Location.Find( pos ), room ) ) then
		return true
	end
