function GM:CalcMainActivity(ply, velocity)
    ...
    if (self:HandlePlayerNoClipping(ply, velocity) or self:HandlePlayerDriving(ply) or self:HandlePlayerVaulting(ply, velocity) or self:HandlePlayerJumping(ply, velocity) or self:HandlePlayerDucking(ply, velocity) or self:HandlePlayerSwimming(ply, velocity)) then
    else
    ...
    if model == "models/player/hhp227/kilik.mdl" then
        ...
        if (len2d > 0) and (len2d < 250) and ply:IsOnGround() and not ply:Crouching() then
            ply.CalcIdeal = ACT_MP_RUN
        end
    end
    ...
    if model == "models/player/midna.mdl" then
        ...
        if ply:GetMoveType() == MOVETYPE_NOCLIP and len2d == 0 and not ply:InVehicle() then
...
function GM:HandlePlayerJumping(ply, velocity)
    ...
    if (not ply.m_bJumping and not ply:OnGround() and ply:WaterLevel() <= 0) then
        ...
        elseif (CurTime() - ply.m_fGroundTime) > 0 and velocity.z > 100 then
            ply.m_bJumping = true
...
function GM:HandlePlayerNoClipping(ply, velocity)
    ...
    if (velocity:Length2D() > 10) then
        ...
        if IsValid(weapon) and weapon:GetClass() == "gmt_hands" then
            ply.CalcIdeal = ACT_MP_SWIM
