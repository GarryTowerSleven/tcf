...
local function CreatePoint( position, angle, premag, postmag, props )
	return 
	{
		Pos = position,
		Angle = angle,
...
----
function CURVE:Add(position, angle, premag, postmag, index, nodeProps  )
	assert(position, "Invalid argument #1. Position expected, got nil")

	local point = CreatePoint(position, angle or Angle(), premag or 1, postmag or 1, nodeProps)
...
function CURVE:CalculateKeyPoints(tesselation)
	...
	for i = 0, tesselation * (#self.Points-1) do
		...
		point, forward, right = self:Calculate( 
