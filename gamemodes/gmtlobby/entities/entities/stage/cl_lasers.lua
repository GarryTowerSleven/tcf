...
function ENT:DrawLaser( att, num, bright )
	...
	for i=1, 3 do
		...
		local forward = self:DeriveUpVector(-50 * self.laserTween + spreadH, (num-2.5) + spreadV)
		forward:Rotate(Angle(0, self:GetAngles().y + 180, 0))
		...
		local spriteAngle = (start_pos - end_pos):Angle()

		self:DrawBasicSprite( self.Mat.Flare2, start_pos, spriteAngle, 150, color )
