		...
		timer.Simple( .9, function()
			...
			if IsValid( self.Victim ) && self.Victim:Alive() then
				...
				local effectdata2 = EffectData()
					...
					//effectdata2:SetNormal( self.Owner:GetAngles():Forward() )

				util.Effect( "gib_bloodemitter", effectdata2, true, true )
...
function SWEP:Think()
	...
	if self.State == 2 then
		if !IsValid(self.Spear) || self.Spear:GetPos():Distance(self.Owner:GetPos()) > 400 then
			self:End()
		end
	end
	...
	if self.State == 3 then
		...
		local vect = angle:GetNormal() * 100
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		...
		if IsValid( phys ) then
			...
			phys:ApplyForceCenter( self.Owner:GetAimVector() * 200000 )
