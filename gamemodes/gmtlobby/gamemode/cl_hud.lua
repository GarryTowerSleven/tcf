...
--table.insert( HudToHide, "CHudDamageIndicator" )
...
GTowerHUD.VolumeSlider:SetMinMax( 0, 100 )
GTowerHUD.VolumeSlider:SetDecimals( 0 )
GTowerHUD.VolumeSlider:SetConVar( "gmt_volume_video" )
...
function GTowerHUD.DrawInfo()
	...
	-- Location
	y = y + 24
	local location = string.upper( LocalPlayer():LocationName() )
...
function GTowerHUD.DrawAmmo()
	...
	local activeSlot = GTowerItems:GetActiveSlot()
	
	-- if we have uses, lets override the clip
	if !activeSlot || !activeSlot.Item || !activeSlot.Item.UsesLeft || activeSlot.Item.UsesLeft <= 0 then return end

	currentMag = activeSlot.Item.UsesLeft
	currentMax = activeSlot.Item.MaxUses
