	...
	if key == IN_JUMP then
		...
		if ply:IsOnGround() then
			...
			if ply:CanSprint() then
				local num = ( ply:GetNWBool("IsSprintting") && .075 ) || .025
				local clamp = math.Clamp( ply:GetNWFloat("Sprint") - num, 0, 1 )
				ply:SetNWFloat("Sprint",clamp)
			end
...
function GM:UCThink()
		...
		for k, v in pairs( player.GetAll() ) do
			...
			if v != self && b && v:Team() == TEAM_PIGS && v:Alive() then
				...
				if !v:GetNWBool("IsScared") then
					...
					v:Scare( t )
					uc:SetNWFloat("Sprint",uc:GetNWFloat("Sprint")+.05)
	...
	if !uc:IsOnGround() then
		...
		if UCsLastStompPlace[ uc:EntIndex() ] then
			...
			if uc:MovementKeyDown() && ( uc:GetNWBool("IsSprintting") && dist >= 8 ) || ( !uc:GetNWBool("IsSprintting") && dist >= 3 ) && dist < 10 then
				uc.PlayStomp = true
			end
	...
	if uc.Swiping then
		...
		if uc:GetNWFloat("SwipeMeter") != 1 then
			local clamp = math.Clamp( uc:GetNWFloat("SwipeMeter") + .00375, 0, 1 )
			uc:SetNWFloat("SwipeMeter",clamp)
		end
...
function meta:DoubleJump()
	...
	self:SetNWFloat("Sprint",self:GetNWFloat("Sprint") - ( GAMEMODE.DJumpPenalty * ( 1 + ( self:GetNWInt("DoubleJumpNum") * .66 ) ) ))
...
function meta:CanDoTailSwipe()
	...
	return (self:Alive() && !self:GetNWBool("IsRoaring") && !self:GetNWBool("IsBiting") && CurTime() >= ( ( self.SwipeTime || 0 ) + .15 ) && self:GetNWFloat("SwipeMeter") >= .25)
...
if SERVER then
	...
	function GM:DrawSwipeMeter( x, y, w, h )
		...
		local diff = math.abs( ply.SwipeMeterSmooth - ply:GetNWFloat("SwipeMeter") )

		ply.SwipeMeterSmooth = math.Approach( ply.SwipeMeterSmooth, ply:GetNWFloat("SwipeMeter"), FrameTime() * ( diff * 5 ) )
