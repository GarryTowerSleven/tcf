...
function meta:CanTaunt()
	...
	if (!self:GetNWBool("IsTaunting") && !self:GetNWBool("IsScared") && !self:GetNWBool("IsSprintting") && self:IsOnGround() && self:Team() == TEAM_PIGS) then
		return true
	end
...
if SERVER then
	...
	function meta:UpdateSpeeds()
		...
		if (self:GetNWBool("IsSprintting") || self:GetNWBool("IsScared")) then
