	...
	if locName then
		...
		if Location.Is( LocalPlayer():Location(), "duels" ) then
			...
			if CamFound then
				...
				duelCam.SpinAngle = duelCam.SpinAngle + FrameTime() * (speed/8)
				local spinHeight = math.atan(speed * math.pi/ 180) * 180 / math.pi

				local angle = Angle(spinHeight, duelCam.SpinAngle, 0)
				local pos = (DOldPos + -angle:Up() * -1250)
				pos = pos + -angle:Forward() * 4500
