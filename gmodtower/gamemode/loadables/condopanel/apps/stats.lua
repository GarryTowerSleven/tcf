...
function APP:Start()
	...
	for k,v in pairs( gamemodes ) do
		local button = {
			...
			name = NiceGamemodes[k],

			func = function() self:Launch(v, false) end,

		}

		self.homebuttons[k] = button
	end
	...
	/*self.condoUnLockButton = {
		...
		func = function() local owner = GtowerRooms:RoomOwner( LocalPlayer().GLocation - 1 ) owner.GRoomLock = false NetLock(owner,false) self.homebuttons[1] = self.condoLockButton self:Repl3("LockCondo", false) end,
	...
	self.condoLockButton = {
		...
		func = function() local owner = GtowerRooms:RoomOwner( LocalPlayer().GLocation - 1 ) owner.GRoomLock = true NetLock(owner,true)  self.homebuttons[1] = self.condoUnLockButton self:Repl3("LockCondo", true) end,
...
function APP:DrawNewsTicker()
	...
	local text = [[Welcome to the GMTower: Deluxe leaderboards viewer. Click on a gamemode to view the leaderboards and statistics.]]
