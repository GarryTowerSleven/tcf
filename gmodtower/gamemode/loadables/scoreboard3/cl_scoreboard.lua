...
function PLAYERS:Init()
	...
	for id, name in pairs( self.TabNames ) do
		...
		if !string.StartWith(game.GetMap(),"gmt_lobby") and table.HasValue( self.LobbyOnlyTabs, name ) then continue end
...
function PLAYERS:PerformLayout()
	...
	for _, tab in pairs( self.Tabs ) do
		...
		if !string.StartWith(game.GetMap(),"gmt_lobby") and table.HasValue( self.LobbyOnlyTabs, tab.Name ) then continue end
...
function PLAYERS:GetPlayerList( tabname, count )
	...
	if tabname == "VIP" then
		...
		for _, ply in ipairs( FilteredPlayerList( player.GetAll() ) ) do
			...
			if ( ply.IsVIP ) and not ( ply:IsAdmin() ) then
...
function PLAYER:CanClick()
	...
	return self:IsMouseInWindow() && self.Player != LocalPlayer() && !Scoreboard.Customization.PlayerActionBoxAlwaysShow && !self.Player.IsLoading && string.StartWith(game.GetMap(),"gmt_lobby")
...
function PLAYERINFO:Update()
	...
	if self.Player.GetNWBool and self.Player:GetNWBool("IsDueling") then

		self:SetupRightSubtitle()
		self.SubtitleRight:SetText( "Dueling with " .. self.Player:GetNWString( "DuelOpponent" ) )

	elseif !self.Player:IsAdmin() and CheckFriendCache(self.Player) then
