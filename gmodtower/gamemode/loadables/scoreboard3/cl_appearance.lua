...
-- TAB
hook.Add("ScoreBoardItems", "AppearanceTab", function() return vgui.Create("AppearancePlayersTab") end)
TAB = {}
TAB.Order = 2
...
local function IconSetModel(icon)
    ...
    if IsLobby and not IsValid(LocalPlayer():GetVehicle()) then
...
function APPEARANCE:UpdateModelPanel()
    if not ValidPanel(self.ModelPanel) then return end
...
function APPEARANCE:GenerateModelSelection()
    ...
    local function AddSpawnIcon(name, model, skin, onclick, category, hatid, hatSlot, description, bettername)
        if not hatid and table.HasValue(UsedModels, model) then return end

        if hatid ~= 0 then
            if table.HasValue(UsedHats, hatid) then return end
            table.insert(UsedHats, hatid)
        else
            if hatid ~= 0 and table.HasValue(UsedModels, model) then return end
        end
        ...
        if name == "Cap Reversed" then
            path = path .. "_backwards"
        end
        ...
        --icon:SetIconSize( 64 )
        icon.HatID = hatid
        icon.UniqueName = GTowerHats.Hats[hatid].unique_Name
        ...
        icon.OnCursorEntered = function()
            if GTowerItems then
                GTowerItems:ShowTooltip(icon.BetterName, icon.Description, self)
            end
        end
        ...
        icon.PaintOver = function(self, w, h)
            ...
            --[[GTowerHats.IsWearingID( LocalPlayer(), icon.HatID )]]
            if (icon.Model == LocalPlayer():GetTranslatedModel()) or (icon.HatID ~= 0 and GTowerHats:IsWearing(LocalPlayer(), icon.UniqueName)) then
            ...
            if not icon:IsMouseOver() then return end
    ...
    for name, model in pairs(player_manager.AllValidModels()) do
        if not LocalPlayer()._ModelTable then
            LocalPlayer()._ModelTable = {}
        end

        if not GTowerModels.NormalModels[name] and not table.HasValue(LocalPlayer()._ModelTable, model) then
            if string.StartWith(name, "medic") or string.StartWith(name, "hostage") or string.StartWith(name, "dod_") then continue end
            if name == "kdedede_pm" or name == "bond" or name == "classygentleman" or name == "maskedbreen" or name == "windrunner" or name == "grayfox" then continue end
            AddSpawnIcon(name, model, skin, IconSetModel, CategoryList, 0, 0, "A standard player model.")
        end
    end
    ...
    -- HATS
    -- Hat remove
    AddSpawnIcon("No Hat", GTowerHats.Hats[0].model, 0, function()
        RunConsoleCommand('gmt_sethat', '0', '1')
    end, HatCategoryListHead, 0, 1, "Remove head hat.")

    AddSpawnIcon("No Hat", GTowerHats.Hats[0].model, 0, function()
        RunConsoleCommand('gmt_sethat', '0', '2')
    end, HatCategoryListFace, 0, 2, "Remove face hat.")
    ...
    table.sort(HatsSorted, function(a, b) return a.Name < b.Name end)

    -- Add the icons
    for id, hat in pairs(HatsSorted) do
        if hook.Call("CanWearHat", GAMEMODE, LocalPlayer(), hat.unique_Name) == 1 then
            if hat.slot == 1 then
                AddSpawnIcon(hat.Name, hat.model, hat.ModelSkinId, IconSetHatModel, HatCategoryListHead, hat.hatid, hat.slot, hat.description, hat.name)
            else
                AddSpawnIcon(hat.Name, hat.model, hat.ModelSkinId, IconSetHatModel, HatCategoryListFace, hat.hatid, hat.slot, hat.description, hat.name)
            end
        end
    end
end
...
function APPEARANCE:GenerateColorSelection()
    ...
    PlayerColor.OnActivePanelChanged = function()
        timer.Simple(0.1, UpdateFromConvars)
    end
    ...
    if LocalPlayer().IsVIP and LocalPlayer():IsVIP() then
        ...
        GlowColor.OnActivePanelChanged = function()
            timer.Simple(0.1, UpdateFromConvars)
        end
...
hook.Add("PlayerThink", "SyncPlayerGlow", function(ply)
    if LocalPlayer().IsVIP and LocalPlayer():IsVIP() then
        if ply._NextGlow and ply._NextGlow < CurTime() then
...
function APPEARANCE:Think()
    ...
    if not self._NextGenerate or self._NextGenerate < CurTime() then
        self:UpdateModelPanel()
        self:GenerateModelSelection()
        self:InvalidateLayout()
...
function SETTINGSCATEGORY:Init()
    ...
    self:SetTabCurve(0)
    self:SetColors(Scoreboard.Customization.ColorDark, Scoreboard.Customization.ColorBackground, Scoreboard.Customization.ColorBright, Scoreboard.Customization.ColorBright)
    self:SetPadding(2)
