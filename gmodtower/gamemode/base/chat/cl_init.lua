...
GTowerChat.BGColor = Color( 26, 40, 117, 180)
GTowerChat.ScrollColor = Color( 19, 28, 81, 215)
//GTowerChat.ChatFont = "GTowerHUDMain"
GTowerChat.ChatFont = "ChatVerdana16"
GTowerChat.ChatURLFont = "ChatUrlVerdana16"
...
function GM:StartChat( TeamSay )
	...
	if Location && Location.IsTheater( LocalPlayer():Location() ) then
...
function GM:ChatText(pID, pName, Text, InternalType, Type)
	...
	if CheckBlockedCache( ply ) then return end
...
table.uinsert( GtowerHudToHide, "CHudChat" )

hook.Add("UpdateAnimation", "Chatting", function( ply )
	if CLIENT && ply:GetNWBool("Chatting") && ( emote && !emote.IsEmoting( ply ) ) then
