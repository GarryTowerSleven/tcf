...
function SCOREBOARD:Init()
	...
	if not string.StartWith(game.GetMap(),"gmt_lobby") then
	...
	*/
	...
	for _, itemFunc in pairs( items ) do
		...
		if success then
			...
			if tab.HideForGamemode && !string.StartWith(game.GetMap(),"gmt_lobby") then
				tab:SetVisible( false )
			elseif tab.OnlyForGamemode && string.StartWith(game.GetMap(),"gmt_lobby") then
				tab:SetVisible( false )
			else
				self:AddTab( tab )
			end
...
local function GetNiceMapName( map )
	return string.upper( Maps.GetName( map ) || game.GetMap() ) or nil
end
