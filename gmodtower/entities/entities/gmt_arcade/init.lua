...
function ENT:Think()
	...
	// Dirty fix for Lobby 2, replaces all offline machines with other ones that do work.

	if self:GetSkin() == 0 then self:SetSkin(17) end
	if self:GetSkin() == 2 then self:SetSkin(18) end
	if self:GetSkin() == 5 then self:SetSkin(21) end
	if self:GetSkin() == 6 then self:SetSkin(22) end
	...
	for _, ply in pairs( player.GetAll() ) do
		if ply:GetPos():WithinDistance( EntPos, 512 ) then
